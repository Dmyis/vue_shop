<template>
  <div>
    <!-- 面包屑导航 -->
    <crumbs-nav>
      <template v-slot:no1>订单管理</template>
      <template v-slot:no2>订单列表</template>
    </crumbs-nav>
      ...
      <el-table :data="orderList" stripe border>
        ...
        <el-table-column label="是否付款" prop="pay_status" width="100px">
          <template v-slot="scoped">
            <el-tag type="success" v-if="!scoped.row.pay_status">已付款</el-tag>
        ...
        <el-table-column label="下单时间" prop="create_time" width="200px">
          <template v-slot="scoped">{{scoped.row.create_time | formatDate}}</template>
        </el-table-column>
